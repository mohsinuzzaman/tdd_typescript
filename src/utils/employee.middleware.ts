...
class EmployeeMiddleware {
  async validateRequiredEmployeeBodyFields(
    req: express.Request,
  ...
  async validateSameEidDoesntExist(
    req: express.Request,
    res: express.Response,
    next: express.NextFunction
  ) {
    ...
    if (user) {
      res
        .status(201)
        .send({
          message: `Employee with given eid ${req.body.eid} already exists`,
        });
    } else {
      next();
    }
  ...
  async validateEmployeeExists(
    ...
    if (emp) {
      ...
      res
        .status(201)
        .send({
          message: `Employee with given eid ${req.params.employeeId} doesn't exists`,
        });
    }
  }
}
